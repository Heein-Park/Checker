    ...
    const connect = async () => {
      ...
      connectToAccount().then(({seedSet, contract, isMinter, address}) => {
        ...
        if (isMinter) { // Minter Conditional Start   
          ...
          this.info = createDiv(`
            <p>Store a preview image and metadata json file to Pinata Cloud and pin the assets in IPFS via Pinata API.</p>
            <p>This process requires JWT or API keys to proceed.</p>
            <h5>Pattern Info</h5>
          ...
          const proceed = async () => { // Proceed Callback
            try {
              ...
              // Opensea Compatibie Metadata
              metadata.name = metadata.title;
              metadata.external_url = "https://blog.heez.in/checkerscopic";
              metadata.image = metadata.properties.image.value;
              metadata.description = "Watch the crystal voxel mass, textured with a unique checker pattern generated by a specific random seed. Exchange the random seed via Opensea to fix the pattern texture over the voxel mass.";
              metadata.attributes = [
                {
                  "trait_type": "Seed", 
                  "value": metadata.properties.seed.value
                },
              ];
        ...
        this.connectBttn.removeClass('loading');
        console.error(err);
      });
    };
